...
<h2>Основы ООП</h2>
    ...
    <ul>
        ...
        <li><div class="spoiler_v2">
                ...
                <div class="spoiler-block">
                    <p>Наследование — это когда класс-наследник имеет все свойства и методы родительского класса, и, как правило, добавляет какой-то новый функционал или/и свойства.</p>
                </div>
            </div></li>
    </ul>
    ...
    <h3>Ассоциация, композиция и агрегация</h3>
    <p><span class="like_b">Ассоциация</span> – это когда один класс включает в себя другой класс в качестве одного из полей. Ассоциация описывается словом «имеет». Автомобиль имеет двигатель. Вполне естественно, что он не будет являться наследником двигателя (хотя такая архитектура тоже возможна в некоторых ситуациях).</p>
    <p>Выделяют два частных случая ассоциации: <span class="like_b">композицию и агрегацию</span>.</p>
    <p><span class="like_b">Композиция</span> – это когда двигатель не существует отдельно от автомобиля. Он создается при создании автомобиля и полностью управляется автомобилем. В типичном примере, экземпляр двигателя будет создаваться в конструкторе автомобиля.</p>
    <p><span class="like_b">Агрегация</span> – это когда экземпляр двигателя создается где-то в другом месте кода, и передается в конструктор автомобиля в качестве параметра.</p>
    <h2>Свойства и  методы</h2>
    <p><span class="like_b">Класс</span> - это базовое понятие в объектно-ориентированном программировании (ООП). Классы образуют синтаксическую базу ООП. Их можно рассматривать как своего рода "контейнеры" для логически связанных данных и функций (обычно называемых методами — см. ниже). Если сказать проще, то класс - это своеобразный тип данных.</p>
    <p><span class="like_b">Экземпляр класса</span> - это объект. <span class="like_b">Объект</span> - это совокупность данных (свойств) и функций (методов) для их обработки. Свойства и методы называются членами класса. Вообще, объектом является все то, что поддерживает инкапсуляцию.</p>
    ...
    <div class="spoiler_v2">
        ...
        <div class="spoiler-block">
<pre><code>// Создаем новый класс Coor:
class Coor {
// данные (свойства):
var $name;
var $addr;
...
$object = new Coor;
    ...
    <p><span class="like_b">Магические методы PHP</span> позволяют разработчикам совершать удивительные трюки в объектно-ориентированном PHP. Магические методы срабатывают в ответ на определенные действия или события.</p>
    ...
    <p>В общем, <span class="like_b">магические методы PHP</span> — это предопределенные методы, выполняемые при каком-либо конкретном событии, они автоматически вызываются, без вызова функции, когда удовлетворяется какое-либо из заданных условий. Например, __construct всегда вызывается при создании экземпляра класса. Магические методы весьма полезны, они предоставляют удивительный функционал для объектно-ориентированного программирования на PHP.</p>
    <h3>Список всех магических методов:</h3>
    <ul>
        <li>__construct</li>
        <li>__destruct</li>
        <li>__call</li>
    ...
    <p>__construct и __destruct – самые популярные методы, которые реализуют базовые понятия объектно-ориентированного программирования: конструктор и деструктор;</p>
    <p>__call, __callStatic, __get и __set – методы, связанные с перегрузкой обращений как к свойствам, так и к методам. Методы __get() и __set() вызываются при установке и получении значения свойства, а методы __call() и __callStatic – при вызове метода. Стоит заметить, что эти магические функции будут вызываться только и исключительно в том случае, если запрошенные метод или свойство не существуют;</p>
    <p>__isset – метод, срабатывающий при вызове функций empty() или isset() на несуществующем или недоступном свойстве класса;</p>
    <p>__unset – срабатывает при вызове функции unset() на несуществующем или недоступном свойстве класса;</p>
    <p>__sleep и __wakeup – методы, которые вызываются только из функций serialize и unserialize соответственно. Метод __sleep будет вызван сразу при применении к объекту функции serialize, а метод __wakeup – при применении unserialize. В настоящий момент методы применяются для сохранения текущего состояния системы с последующим восстановлением данного состояния (например, коннект к базе);</p>
    <p>__toString – метод, с помощью которого можно обращаться к классу как к строке (например, с помощью print или echo);</p>
    <p>__set_state – метод, который вызывается для классов, экспортирующих значения свойств функцией var_export();</p>
    <p>__clone – вызывается при клонировании объекта (введен для использования из-за того, что объекты в php5 и выше передаются по ссылке);</p>
    <p>__invoke – вызывается при попытке использовать объект в качестве функции.
